...
if exist(['wintx_' num2str(kk_first)]),
    ...
    disp(horzcat('Window size for corner finder. wintx = ',int2str(dx_in),' and winty = ',int2str(dy_in)));
    wintx = dx_in;
    if isempty(wintx), wintx = wintx_default; end;
    wintx = round(wintx);
    winty = dy_in;
...
if ~dont_ask,
    fprintf(1,'Using the automatic square counting mechanism\n');
    manual_squares = [];
